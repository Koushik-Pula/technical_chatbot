...
class porter_stemmer:
    ...
    def __init__(self, tokenised_phrase) -> None:
        self.tokenised_phrase = tokenised_phrase

    def get_stemmed_tokens(self):
        stemmed_tokens = []
        for wordz in self.tokenised_phrase:
            if len(wordz) > 2:
                word = self.step_1a(wordz)
                # print(word)
                ...
                stemmed_tokens.append(word)  # NUMBER OF WORDS PASSING THROUGH STEMMER  
    ...
    def CVC(self, stem):
        stem = stem.lower()
        if len(stem) < 3:  # Ensure stem has at least 3 characters
            return False
        pattern = self.form(stem)
